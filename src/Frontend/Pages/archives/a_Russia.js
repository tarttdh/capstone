...
export class ArcRussia extends Component {
  constructor(props) {
    super(props);
    this.state = {
      error: null,
  ...
  componentDidMount() {
    ...
    fetch(`${serverUrl}/Ukraine_articles/?format=json&Date__gte=${startDate.toISOString()}&Date__lt=${threeMonthsAgo.toISOString()}`)
      .then((res) => res.json())
      .then(
        (result) => {
